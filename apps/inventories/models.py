...
class UserInventory(BaseModel):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="inventories", help_text="이 리소스를 소유한 사용자")
    ...
    region_normalized = models.CharField(max_length=10, choices=NormalizedRegion.choices, help_text="정규화된 리전명")
    ...
    cpu_usage_avg = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, help_text="최근 7일 평균 CPU 사용률 (%) - 과스펙 판단 기준"
    )
    memory_usage_avg = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, help_text="최근 7일 평균 메모리 사용률 (%)"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(fields=["user", "provider", "resource_id"], name="unique_user_resource"),
        ]
